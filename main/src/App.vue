<template>
  <div class="layout-wrapper">
    ...
    <!-- 子应用挂载点 -->
    <div id="subapp-viewport" v-show="zhu"></div>
    <router-view v-if="!zhu"></router-view>
    <!-- 主应用的路由需要重现写,子应用需要v-show，缓存数据 -->
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    //通过重写 history.pushState 方法，在路由发生变化时触发自定义事件 pushState，
    // 并将事件传递给 window.dispatchEvent 方法。同时，还监听了 popstate 事件。
    // 这样，通过自定义事件的方式，可以在主应用或子应用中实现消息传递和触发特定操作。
    listenRouterChange () {
      const _wr = function (type) {
        ...
        return function () {
          ...
          // 主应用可以使用 window.dispatchEvent(event) 方法触发自定义事件，
          // 子应用可以通过监听 window.addEventListener(eventType, listener) 来接收事件并执行相应的逻辑。
